...
using static System.Linq.Expressions.ExpressionStubs1;
...
namespace Microsoft.CSharp.Expressions
{
    ...
    public sealed partial class NewMultidimensionalArrayInitCSharpExpression : CSharpExpression
    {
        ...
        public override Expression Reduce()
        {
            ...
            var consts = Bounds.Select(x => Expression.Constant(x)).ToArray();

            exprs[0] = Expression.Assign(res, Expression.NewArrayBounds(Type.GetElementType(), consts));
            ...
            for (var i = 1; i <= n; i++)
            {
                ...
                var indexes = indexValues.Select(x => Expression.Constant(x));
