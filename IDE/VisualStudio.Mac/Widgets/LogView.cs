...
namespace VisualStudio.Mac
{
    public class LogView : Gtk.VBox
    {
        ...
        internal LogViewProgressMonitor GetProgressMonitor(bool clearConsole)
        {
        ...
        void UpdateCustomFont()
        {
            ...
            {
                textEditorControl.ModifyFont(IdeApp.Preferences.CustomOutputPadFont);
            }
        }
        ...
        {
            WriteDebug(null, message, category);
        }

        public void WriteDebug(LogViewProgressMonitor monitor, string message, string category = null)
        {
    ...
    public class LogViewProgressMonitor : OutputProgressMonitor
    {
        ...
        public LogTextWriter InfoLog { get; private set; }  = new LogTextWriter();
        public LogTextWriter DebugLog { get; private set; } = new LogTextWriter();
        public LogTextWriter WarningLog { get; private set; } = new LogTextWriter();
        ...
        void DebugLog_TextWritten(string writtenText)
        {
            if (CheckPadValid())
                outputPad.WriteDebug(this, writtenText);
        }

        void WarningLog_TextWritten(string writtenText)
        {
            if (CheckPadValid())
                outputPad.WriteWarning(this, writtenText);
        }


        void InfoLog_TextWritten(string writtenText)
        {
            if(CheckPadValid())
                outputPad.WriteConsoleLogText(this, writtenText);
        }
        ...
        protected override void OnWriteErrorLog(string message)
        {
            if (CheckPadValid())
            {
                outputPad.WriteError(this, message);
            }
        }
        ...
        class LogViewProgressConsole : OperationConsole
        {
            ...
            public override void Debug(int level, string category, string message)
            {
                ...
                {
                    monitor.outputPad.WriteDebug(monitor, message, category);
                }
            }
